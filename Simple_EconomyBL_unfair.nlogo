...
to go
  ...
  ask turtles with [ (wealth > 5 ) and (wealth <= 30) ] [transact_poor ]
  ask turtles with [ (wealth > 30 ) and (wealth <= 80) ] [transact_medium]
  ask turtles with [  wealth > 80 ]  [transact_rich]
  ;; prevent wealthy turtles from moving too far to the right
  ;; in the model setting box, we activate the option world wraps horizontally
  ;; otherwise message error.
...
to transact_poor
  set wealth wealth - 5
  ask one-of other turtles [ set wealth wealth + 5 ]
end
to transact_medium
  set wealth wealth - 2
...
;; to check the money conservation
to-report rest-wealth
  report sum [wealth] of max-n-of (count turtles) turtles [ wealth] -
  sum [ wealth ] of max-n-of (count turtles * 0.10) turtles [ wealth ]-
  sum [ wealth ] of min-n-of (count turtles * 0.50) turtles [ wealth ]
end
...
"rest" 1.0 0 -7500403 true "" "plot rest-wealth"
...
852
358
wealth between bottom 50 % and top 10%
rest-wealth
1
1
11
...
This model is based on the simple economy model which  is from Chapter Two of the book "Introduction to Agent-Based Modeling: Modeling Natural, Social and Engineered Complex Systems with NetLogo", by Uri Wilensky & William Rand.

* Wilensky, U. & Rand, W. (2015). Introduction to Agent-Based Modeling: Modeling Natural, Social and Engineered Complex Systems with NetLogo. Cambridge, MA. MIT Press.

This model is mostly based on the economy model.

## WHAT IS IT?

This model is a modification of the original economy model from Netlogo.

It is a thought experiment of  a world where, in every time steps depending
on the current wealth, each agent gives a certain amount of money to one
other person (at random) if they have any money to give.
If a person has a wealth between 5 and 30 $, the person is considered as poor.
for a wealth between 30 and 80 $, the person is considered as medium wealth
and for has a wealth larger than 80 $, the person is considered as rich.

If a person is in the poor class, the person will give 5 $ to one other person
(at random). Similarly, if a person is in the medium class, the person will
give 2 $ to one other person.
If the person is rich, she will give 1 $ to one other person.
If they have no money then they do not give out any money.

## HOW IT WORKS

The SETUP for the model creates 500 agents, and then gives them each 100 dollars.
At each tick, they give a certain amount of money (5, 2 or 1) depending in which
classes they belong (poor, medium, or rich).  If they have no money then they 
do nothing. Each agent also moves to an x-coordinate equal to its wealth.
...
Change the basic transaction rule of the model.How about if they give a random amount to another agent at each tick? 

Try with the possibily of negative wealth (debts) and interests.
In this case the money is not conserved anymore.
...
To avoid a message error in the first graphic windows (model setting), 
we have activated the option : world wraps horizontally.
...
@#$#@#$#@
<experiments>
  <experiment name="experiment_unfair" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
